...
resource "azurerm_resource_group" "feed" {
    name = "ListApp-FeedService-${var.pr_number}"
    location = "Central  US"
}

resource "azurerm_app_service" "feed" {
  name                = "feedservice-dev-${var.pr_number}"
  ...
  site_config {
    linux_fx_version = "DOCKER|${data.azurerm_container_registry.general.login_server}/feedservice-pr:${var.image_name}"
    always_on = "true"
  }
